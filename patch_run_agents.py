...
text = text.replace(old_cat.strip(), new_cat.strip())

# 2. process_drive_file_to_post
...
text = text.replace(old_writer.strip(), new_writer.strip())
...
text = text.replace(old_eval.strip(), new_eval.strip())

# 4. category pass
old_run = """        # Editor Agent를 통해 SEO 최적화 및 최종 형식 교정
        editor_data = run_editor(title_temp, raw_html)"""

new_run = """        # Editor Agent를 통해 SEO 최적화 및 최종 형식 교정
        editor_data = run_editor(title_temp, raw_html)
        editor_data["category"] = category"""

text = text.replace(old_run.strip(), new_run.strip())
